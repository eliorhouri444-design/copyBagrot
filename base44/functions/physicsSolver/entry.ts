...
Deno.serve(async (req) => {
    try {
        ...
        // Cache check
        if (useCache) {
            const questionHash = `physics_${question.substring(0, 100)}`;
        ...
        console.log('⚛️ Solving physics problem...');
        ...
        const messages = [
            {
                role: "system",
                content: `אתה מומחה פיזיקה לבגרות (${units} יח').

**החזר JSON:**
{
  "phenomenon": "קינמטיקה/דינמיקה/חשמל/אופטיקה/גלים",
  "given_data": [{"parameter": "מהירות", "value": "10", "units": "m/s"}],
  "required": "מה מבוקש",
  "fbd": "דיאגרמת כוחות",
  "laws_used": ["חוק ניוטון השני"],
  "steps": [
    {
      "number": 1,
      "title": "...",
      "physics_explanation": "הסבר פיזיקלי",
      "formula": "LaTeX",
      "calculation": "חישוב",
      "result": "תוצאה + יחידות"
    }
...
דיוק מוחלט! תמיד כלול יחידות!`
        ...
        console.log('✅ Physics solution completed');
